...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Khởi tạo TinyMCE
    if (document.querySelector('textarea#product-description')) {
        tinymce.init({
    ...
    if (!generateBtn || !variantListContainer || !modalEl) { return; }

    const attributesModal = new bootstrap.Modal(modalEl);
    ...
    function generateCombinations(arrays) {
        ...
        for (let i = 1; i < arrays.length; i++) {
            const nextResult = [];
            result.forEach(currentCombo => {
                arrays[i].forEach(item => nextResult.push([...currentCombo, item]));
            });
            result = nextResult;
        }
        return result;
    ...
    generateBtn.addEventListener('click', function() {
        ...
        modalEl.querySelectorAll('.attribute-value-checkbox:checked').forEach(checkbox => {
        ...
        const combinations = generateCombinations(Object.values(selectedValuesByAttribute));
        ...
        const tableHtml = `
            <table class="table table-bordered align-middle">
                <thead class="table-light"><tr><th style="width: 35%;">Tên phiên bản</th><th style="width: 25%;">Giá *</th><th style="width: 20%;">Tồn kho *</th><th style="width: 20%;">Ảnh (Tùy chọn)</th></tr></thead>
                <tbody>
                    ${combinations.map((combo, index) => {
                        const variantName = combo.map(v => v.name).join(' / ');
                        const attributeValueIds = combo.map(v => v.id);
                        return `<tr><td><strong>${variantName}</strong>${attributeValueIds.map(id => `<input type="hidden" name="variants[${index}][attribute_value_ids][]" value="${id}">`).join('')}<input type="hidden" name="variants[${index}][name]" value="${variantName}"></td><td><input type="number" name="variants[${index}][price]" class="form-control form-control-sm" required></td><td><input type="number" name="variants[${index}][stock]" class="form-control form-control-sm" required></td><td><input type="file" name="variants[${index}][image]" class="form-control form-control-sm"></td></tr>`;
                    }).join('')}
                </tbody>
            </table>`;
        
        variantListContainer.innerHTML = tableHtml;
