...
@push('scripts')
    {{-- <script>
        $(document).ready(function() {
            ...
            $('#discount-code').on('input', function() {
                ...
                if (appliedDiscount > 0 && currentCode !== lastDiscountCode) {
                    resetDiscount();
                    showDiscountMessage('', '');
                }
            });
    ...
    <script>
        $(document).ready(function() {
            let originalTotal = {{ (int) $cart->total_price }};
            ...
            function updateTotal() {
                ...
                $('#final-total').val(finalTotal); // Cập nhật hidden input để gửi về controller
                $('#discount-code-hidden').val(currentDiscount ? currentDiscount.code : '');
                $('#discount-value-hidden').val(currentDiscount ? currentDiscount.discountValue : 0);
                $('#final-total-hidden').val(finalTotal);
            }

            // Hiển thị thông báo
            function showDiscountMessage(message, type) {
                $('#discount-message').html('<div class="alert alert-' + type + '">' + message + '</div>');
            }
            ...
            $('#apply-discount-btn').click(function(e) {
                ...
                $.ajax({
                    url: '{{ route('cart.apply-discount') }}', // có thể đổi sang route validateDiscount nếu muốn
                    method: 'POST',
                    ...
                    success: function(data) {
                        if (data.success) {
                            ...
                            let discountValue = data.discount.discount_type === 'amount' ?
                            ...
                            // Cập nhật hiển thị
                            $('#applied-code').text(currentDiscount.code);
                            $('#discount-value').text(
                            ...
                            $('#discount-amount').text(
                                `-${formatCurrency(currentDiscount.discountValue)}`);

                            updateTotal();
                            showDiscountMessage('Áp dụng mã giảm giá thành công!', 'success');
                            $('#discount-code').val('');
                        } else {
                            showDiscountMessage(data.error || 'Mã giảm giá không hợp lệ!',
                                'danger');
                            resetDiscount();
                        }
                    },
                    error: function() {
                        showDiscountMessage('Có lỗi xảy ra, vui lòng thử lại!', 'danger');
                    },
                    complete: function() {
                        $('#apply-discount-btn').prop('disabled', false).text('Áp dụng');
                    }
            ...
            // Khi người dùng thay đổi mã khác → reset giảm giá
            $('#discount-code').on('input', function() {
                const currentCode = $(this).val().trim();
                if (currentDiscount && currentCode !== lastDiscountCode) {
                    resetDiscount();
                    showDiscountMessage('', '');
                }
            });
            ...
            <form action="{{ route('client.checkout.placeOrder') }}" method="POST">
                ...
                <div class="row">
                    ...
                    <div class="col-lg-7 checkout-form">
                        ...
                        @guest
                            <p>Bạn đã có tài khoản? <a href="{{ route('login') }}">Đăng nhập</a> để điền thông tin nhanh hơn.
                            </p>
                        @endguest
                        ...
                        <div class="checkout-section">
                            ...
                            <div class="discount-container">
                                <div class="input-group discount-input-group">
                                    <input type="text" id="discount-code" class="form-control discount-input"
                                    ...
                                    <div class="payment-option">
                                        ...
                                        <label class="form-check-label payment-label" for="payment_momo">
                                            <div class="payment-content">
                                                <div class="payment-icon">
                                                    {{-- Sử dụng icon Momo --}}
                                                    <img src="https://developers.momo.vn/v3/vi/assets/images/logo.svg"
                    ...
                    <div class="col-lg-5">
                        <div class="order-summary">
                                        ...
                                        <div class="item-details">
                                            ...
                                            <div class="item-info">
                                                <div class="product-name">
                                                    {{ $item->variant->product->name ?? 'Sản phẩm không tồn tại' }}</div>
                            ...
                            <div class="summary-row summary-total">
                                <span>Tổng cộng:</span>
                                <input id="final-total" type="hidden" name="final_total"
                                    value="{{ $cart->total_price }}">
                                <span
                                    id="final-total-display">{{ number_format($cart->total_price, 0, ',', '.') }}đ</span>
                            </div>
